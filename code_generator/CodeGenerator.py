...
void update_SGD(float learning_rate){\n"""
    ...
    def codeGeneration(self):
        ...
        if gen_kernels:
            op_gen = OpGenerator(include_path, source_path,
                                 self.MemSche.layer, self.fp_requantize)
            op_gen.genOpcode()
    ...
    for (i = 0; i < """
        ...
        for (j = 0; j < """
            ...
            int start_x = TN_MAX("""
                + str(first_width -
                      self.patch_params["pad_l"] - self.patch_params["pad_r"])
                + """ * j - """
                + str(self.patch_params["pad_l"])
                + """,0);
            int start_y = TN_MAX("""
...
"""
            ...
            for i, op in enumerate(schedule.layer):
                ...
                if "is_patch" not in layer_info or not layer_info["is_patch"]:
                    break  # end of patch-based
                string = "/* layer " + \
                    str(layercnt) + ":" + layer_info["op"] + " */\n"
                layercnt += 1
                fp.write(string)
                if layer_info["op"] == "CONV_2D":
                    if self.unsigned_input:
                        raise Exception(
                            "unsigned input is not supported by patch-based yet")
...
"""
    ...
    def _genMemBuffer(self):
        ...
        string = "#define NNoutput &buffer0[" + \
            str(_findtheinferenceOutput(schedule.layer)) + "];"
        fp = self.header_handle
        fp.write("\n" + string + "\n")

        # activation buffers
        string = "\n/* sram:" + \
            str(schedule.peakmem) + ", flash:" + str(schedule.flash) + " */\n"
        fp.write(string)
        string = "#define PEAK_MEM " + \
            str(schedule.peakmem) + "\n" + \
        ...
        fp.write(string)
        accumulate_ptr = 0
        string = "static signed char *buffer0 = &buffer[" + str(
            accumulate_ptr) + "];\n"
        accumulate_ptr += int(schedule.buffers["input_output"])
        fp.write(string)

        string = "static int16_t *sbuf = (int16_t *)&buffer[" + str(
            accumulate_ptr) + "];\n"
        accumulate_ptr += int(schedule.buffers["im2col"])
        fp.write(string)
        string = "static int16_t *kbuf = (int16_t *)&buffer[" + str(
            accumulate_ptr) + "];\n"
        accumulate_ptr += int(schedule.buffers["kernel"])
        fp.write(string)
        string = "const int SBuffer_size = " + \
            str(int(schedule.buffers["im2col"])) + ";\n"
        fp.write(string)
        string = "const int KBuffer_size = " + \
            str(int(schedule.buffers["kernel"])) + ";\n"
        fp.write(string + "\n")
...
signed char* getOutput() {
        ...
        for i, op in enumerate(schedule.layer):
            ...
            if layer_info["op"] == "CONV_2D":
                ...
                if "bias_name" in layer_info:
                    ...
                    self._parseBias(self.parse_count,
                                    layer_info["bias"].flatten())
                self._parseEffectivescales(
                    self.parse_count, layer_info["effective_scale"].flatten())
    ...
    def _parseCWHWeight(self, Lindex, weight, height, width, channel):
        ...
        # 8bit implementation
        if self.BIT == 8:
            string = "const unsigned char CWHweight" + \
                str(Lindex) + "[" + str(len(weight)) + "] = {"
            fp.write(string)
    ...
    def _parseCHWWeight(self, Lindex, weight, channel):
        ...
        # 8bit implementation
        if self.BIT == 8:
            string = "const unsigned char CHWweight" + \
                str(Lindex) + "[" + str(len(weight)) + "] = {"
            fp.write(string)
            for j in range(channel):
    ...
    def _parseEffectivescales(self, Lindex, scales):
        fp = self.header_handle
        string = "const float scales" + \
            str(Lindex) + "[" + str(len(scales)) + "] = {"
        fp.write(string)
    ...
    def _parseWeight(self, Lindex, weight, weight_name=None, is_const=True):
        ...
        string = f"{const_str}unsigned char weight" + \
            str(Lindex) + "[" + str(len(weight)) + "] = {"
        fp.write(string)
        ...
        if self.is_training:
            string = f"{const_str}float weight_fp" + \
                str(Lindex) + "[" + str(len(weight)) + "] = {"
            fp.write(string)
        ...
        if weight_name is not None:
            for r in self.trainSRAMTable:
                if r.name == weight_name:
                    return
            self.trainSRAMTable.append(tensorRecorder(
                weight_name, len(weight), "unknown"))

            if weight.dtype == "int8":
                string = f"{const_str}unsigned char* {weight_name}=weight" + \
                    str(Lindex) + ";\n"
            else:
                raise NotImplementedError
            fp.write(string)
    ...
    def _parseWeightPartial(self, Lindex, weight, first_k_channel=None, weight_name=None, is_const=True):
        ...
        # weight in SRAM
        string = "unsigned char weight" + \
            str(Lindex) + "[" + str(len(weight_SRAM.flatten())) + "] = {"
        fp.write(string)
        ...
        # weight in Flash
        string = "const unsigned char weight" + \
            str(Lindex) + "Flash[" + str(len(weight_Flash.flatten())) + "] = {"
        fp.write(string)
        ...
        if weight_name is not None:
            ...
            if weight.dtype == "int8":
                string = f"unsigned char* {weight_name}=weight" + \
                    str(Lindex) + ";\n"
                string += f"const unsigned char* {weight_name}Flash=weight" + str(
                    Lindex) + "Flash;\n"
            else:
                raise NotImplementedError
            fp.write(string)
    ...
    def _parseoffsetBias(self, Lindex, bias, input_offset, weight, channel, bias_name=None, is_const=True):
        fp = self.header_handle
        const_str = "const " if is_const else ""
        string = f"{const_str}int32_t offsetBias" + \
            str(Lindex) + "[" + str(len(bias)) + "] = {"
        fp.write(string)
        kernelsize = int(len(weight) / channel)
        ...
        for i in range(channel):
            ...
            fp.write(
                str(self.int32_clip(bias[i] + tmpW * input_offset)) + ", ")
        fp.write("};\n")
        string = f"{const_str}int32_t offsetRBias" + \
            str(Lindex) + "[" + str(len(bias)) + "] = {"
        fp.write(string)
        kernelsize = int(len(weight) / channel)
        for i in range(channel):
            ...
            fp.write(str(bias[i] + tmpW * input_offset -
                     self.int32_clip(bias[i] + tmpW * input_offset)) + ", ")
        fp.write("};\n")

        if bias_name is not None:
            string = f"{const_str}int32_t* {bias_name}=offsetRBias" + \
                str(Lindex) + ";\n"
            fp.write(string)
    ...
    def _parseBias(self, Lindex, bias, bias_name=None, is_const=True):
        fp = self.header_handle
        const_str = "const " if is_const else ""
        string = f"{const_str}int32_t bias" + \
            str(Lindex) + "[" + str(len(bias)) + "] = {"
        fp.write(string)
        ...
        if self.is_training:
            string = f"{const_str}float bias_fp" + \
                str(Lindex) + "[" + str(len(bias)) + "] = {"
            fp.write(string)
        ...
        if bias_name is not None:
            string = f"{const_str}int32_t* {bias_name}=bias" + \
                str(Lindex) + ";\n"
            fp.write(string)

    def _parseRequantize(self, Lindex, shift, multiplier):
        fp = self.header_handle
        string = "const int32_t shift" + \
            str(Lindex) + "[" + str(len(shift)) + "] = {"
        fp.write(string)
        for _, value in enumerate(shift):
            fp.write(str(value) + ", ")
        fp.write("};\n")

        string = "const int32_t multiplier" + \
            str(Lindex) + "[" + str(len(multiplier)) + "] = {"
        fp.write(string)
