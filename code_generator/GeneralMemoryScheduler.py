...
class GeneralMemoryScheduler:
    ...
    def allocateMemory(self):
        ...
        if self.VisaulizeTrainable:
            self.allocator.addTensor(
                0, length_model, trainable, type=TTYPE_STATIC_WEIGHT)
        ...
        for i, op in enumerate(self.layer):
            if (
                ...
                if op.input_tensors[0].allocator_idx == op.output_tensors[0].allocator_idx:
                    self.allocator.rectangles[op.input_tensors[0]
                                              .allocator_idx]["stride2_inplace_idx"] = i
        ...
        for i, op in enumerate(self.layer):
            ...
            for cnt, t in enumerate(op.input_tensors):
                if cnt == 0:
                    op.params["input_buf_add_offset"] = self.allocator.getIdxAddress(
                        t.allocator_idx)
                    op.params["input_buf_add"] = "front"
                elif cnt == 1:
                    op.params["input2_buf_add_offset"] = self.allocator.getIdxAddress(
                        t.allocator_idx)
                    op.params["input2_buf_add"] = "front"
                elif cnt == 2:
                    op.params["input3_buf_add_offset"] = self.allocator.getIdxAddress(
                        t.allocator_idx)
                    op.params["input3_buf_add"] = "front"
                op.input_tensors[cnt].buffer_name = "buffer0"
                op.input_tensors[cnt].buffer_address = self.allocator.getIdxAddress(
                    t.allocator_idx)
            for cnt, t in enumerate(op.output_tensors):
                if cnt == 0:
                    op.params["output_buf_add_offset"] = self.allocator.getIdxAddress(
                        t.allocator_idx)
                    op.params["output_buf_add"] = "front"
                    op.output_tensors[cnt].buffer_name = "buffer0"
                    op.output_tensors[cnt].buffer_address = self.allocator.getIdxAddress(
                        t.allocator_idx)
                if cnt == 1:
                    op.params["output2_buf_add_offset"] = self.allocator.getIdxAddress(
                        t.allocator_idx)
        ...
        self.peakmem = (
            ...
            self.allocator.get_peak() + \
            self.buffers["im2col"] + self.buffers["kernel"]
        )
    ...
    def dumpLayerMem(self):
        ...
        print(
            "----------------------|  activation  |  runtime  |   sum    ||   weight   |   bias   |  scale   |     sum     |     MAC     |"  # noqa: E501
        )
    ...
    def __dumpMemInfo(self, layermem):
        ...
        string += str(maxActive + maxRuntime).ljust(10) + "||"
        ...
        for i, _ in enumerate(layermem):
            ...
            if "activation" in layermem[i]:
                substr = (
                    ...
                    "{:.0%}".format(
                        layermem[i]["activation"] / maxActive) + ")"
                )
            ...
            if "bias" in layermem[i]:
                substr = (
                    str(layermem[i]["bias"]) + " (" + "{:.0%}".format(
                        layermem[i]["bias"] / (totalBias + 0.0001)) + ")"
                )
                string += str(substr).ljust(10) + "|"
                flash += layermem[i]["bias"]
            if "scale" in layermem[i]:
                substr = (
                    str(layermem[i]["scale"]) + " (" + "{:.0%}".format(
                        layermem[i]["scale"] / totalScale + 0.0001) + ")"
                )
                string += str(substr).ljust(10) + "|"
                ...
                if flash > 0:
                    string += (
                        str(flash)
                        + " ("
                        + "{:.0%}".format(flash / (totalWeight +
                                          totalBias + totalScale + 0.0001))
                        + ")"
                    )
                    string = string.ljust(110) + "|"
            # flash end
            if "MAC" in layermem[i]:
                substr = str(
                    layermem[i]["MAC"]) + " (" + "{:.0%}".format(layermem[i]["MAC"] / totalMAC) + ")"
