...
class InputResizer:
    ...
    def inputResize(self, input_h, input_w):
        for i, layer in enumerate(self.layer):
            ...
            previous_layer_info = _find_previous_info(
                self.layer, layer_info["input_idx"])
            ...
            if i == 0:
                ...
                _changeOPTensorSize(
                    self.layer[i], "input", 0, layer_info["input_h"], layer_info["input_w"])
            else:
            ...
            if op_code_str == "CONV_2D" or op_code_str == "DEPTHWISE_CONV_2D":
                layer_info["output_h"] = math.ceil(
                    layer_info["input_h"] / layer_info["stride_h"])
                layer_info["output_w"] = math.ceil(
                    layer_info["input_w"] / layer_info["stride_w"])
                _changeOPTensorSize(
                    self.layer[i], "output", 0, layer_info["output_h"], layer_info["output_w"])
                ...
                _changeOPTensorSize(
                    self.layer[i], "input", 1, layer_info["input2_h"], layer_info["input_w"])
                ...
                layer_info["output_h"] = int(
                    layer_info["input_h"] / layer_info["filter_h"])
                layer_info["output_w"] = int(
                    layer_info["input_w"] / layer_info["filter_h"])
                layer_info["output_c"] = layer_info["input_c"]
                _changeOPTensorSize(
                    self.layer[i], "output", 0, layer_info["output_h"], layer_info["output_w"])
    ...
    def patchResize(self, PatchLayers, PatchSize, PatchSize_height):
        for i, layer in enumerate(self.layer):
            ...
            if i < PatchLayers:
                ...
                if i == 0:
                    ...
                    _changeOPTensorSize(
                        self.layer[i], "input", 0, PatchSize_height, PatchSize)
                else:
                ...
                if op_code_str == "CONV_2D" or op_code_str == "DEPTHWISE_CONV_2D":
                    layer_info["output_h"] = math.ceil(
                        (layer_info["input_h"] - layer_info["kernel_h"] +
                         1) / layer_info["stride_h"]
                    )
                    layer_info["output_w"] = math.ceil(
                        (layer_info["input_w"] - layer_info["kernel_w"] +
                         1) / layer_info["stride_w"]
                    )
                    _changeOPTensorSize(
                        self.layer[i], "output", 0, layer_info["output_h"], layer_info["output_w"])
                elif op_code_str == "ADD":
                    ...
                    _changeOPTensorSize(
                        self.layer[i], "input", 0, layer_info["input_h"], layer_info["input_w"])
                    _changeOPTensorSize(
                        self.layer[i], "input", 1, layer_info["input_h"], layer_info["input_w"])
            else:
        ...
        if PatchLayers > 0:
            self.layer[PatchLayers].params["input_idx"] = (
                str(self.layer[PatchLayers].params["input_idx"]
                    ) + "_start_normal_infernece_block"
            )
