def getPatchParams(layers, split_idx, n_patch):
    ...
    resolution = max(layers[0].get_layer_info()["input_h"],
                     layers[0].get_layer_info()["input_w"])
    ...
    patch_params["single_rf"] = compute_receptive_field(
        layers, patch_params["layer_cnt"], 1)
    patch_params["output_c"] = layer_cnt["input_c"]
    patch_params["output_h"] = layer_cnt["output_h"]
    patch_params["output_w"] = layer_cnt["output_w"]
    patch_params["grain_rf"] = compute_receptive_field(
        layers, patch_params["layer_cnt"], grain_size)
    patch_params["grain_rf_height"] = compute_receptive_field(
        layers, patch_params["layer_cnt"], layer_cnt["input_h"] // n_patch
    )
    print("receptive field: single {} all {}".format(
        patch_params["single_rf"], patch_params["grain_rf"]))
