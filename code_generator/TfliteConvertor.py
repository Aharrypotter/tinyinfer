...
class TfliteConvertor(object):
        ...
        for i in range(operators_len):
            ...
            if i + 2 < operators_len - 2:
                ...
                if self.checkIfRequireSEelementmult(three_op_sequence):
                    ...
                    #  DWCONV                                        |            -> MUL |
                    #                                                |   SEelementmult   |
                    SEelementmult_op = TF_Parser.parse_SEelement(
                        three_op_sequence, self.model, self.layer)
    ...
    def _handleOperator(self, op):
        ...
        if op_code_str == "CONV_2D":
            self.layer.append(TF_Parser.parse_conv2d(
                op, self.model, self.tmpPADIndice))
            self.tmpPADIndice = None
            ...
            ret_op = TF_Parser.parse_mead1dto2d(
                op, self.model, self.average_1D_to_2D_holder)
    ...
    def _convert_TRANSPOSE(self, op):
        ...
        # fuse pad into conv
        self.skip_transpose = PAD_tensorIndice(
            input_tensor.tensor_idx, output_tensor.tensor_idx)
