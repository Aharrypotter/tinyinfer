...
class BaseAllocator:
    ...
    def allocate(self):
        ...
        print(
            f"Deriving the memory schedule for {len(self.rectangles)} activation tensors.")
    ...
    def visualize(self, path, train_start_idx=-1, scale=1024):
        ...
        x_ticks = numpy.arange(
            0, FIGURE_CONFIG["X_MAX"] + 1, step=FIGURE_CONFIG["X_STEP"])
        plt.xticks(x_ticks, fontsize=FIGURE_CONFIG["FONT_SIZE"])
        # y ticks
        y_ticks = numpy.arange(
            0, FIGURE_CONFIG["Y_MAX"] + 1, step=FIGURE_CONFIG["Y_STEP"])
        ...
        figure.set_size_inches(
            FIGURE_CONFIG["FIGURE_W_INCH"], FIGURE_CONFIG["FIGURE_H_INCH"])
                ...
                if FIGURE_CONFIG["SHOW_INDEX"]:
                    ...
                    ax.annotate(
                        ...
                        fontsize=_get_index_font_size(
                            FIGURE_CONFIG["INDEX_FONT_SIZE"], (
                                size / scale), FIGURE_CONFIG["Y_STEP"]
                        ),
                        weight="bold",
                        ha="center",
                        va="center",
