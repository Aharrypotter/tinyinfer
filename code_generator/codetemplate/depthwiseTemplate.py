...
"""
    ...
    for (i = 0; i < output_y; i++) {
        ...
        """
        if self.stride != 1:
            retString += "cols_8b_iterptr += " + \
                str(self.pad_w) + " * 2 - (column_x & 1);\n"
            retString += "        cols_8b_iterptr += (STRIDE - 1) * (column_x + " + str(
                self.pad_w) + " * 2);\n"
        else:
