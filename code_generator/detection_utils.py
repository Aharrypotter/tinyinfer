    ...
    def genPostProcessing(self):
        ...
        for key in self.det_conf:
            ...
            for i in range(len(output_config["anchors"]) // 2):
                output_string += "{" + \
                    f"{output_config['anchors'][i * 2]},{output_config['anchors'][i * 2 + 1]}" + "},"
            output_string += "},\n"
        output_string += "};\n"
        # generate the post processing part
        output_string += """void det_post_procesing(int* box_cnt, det_box** ret_box, float threshold){\n"""
        tensorstring = "    int8_t *input_data" + \
            f"[{str(len(self.det_conf))}]" + "={"
        zerostring = "    int8_t y_zero" + \
            f"[{str(len(self.det_conf))}]" + "={"
        scalestring = "    float y_scale" + \
            f"[{str(len(self.det_conf))}]" + "={"
        ...
        for key in self.det_conf:
            ...
            for layer in self.layers:
                if str(layer.params["output_idx"]) == str(target_id):
                    print(
                        f"Processing the output {layer.params['output_idx']} as a detection feature map.")
                    tensorstring += (
                    ...
                    top_avaialble_addrees = max(
                        top_avaialble_addrees, layer.params["output_buf_add_offset"] +
                        layer.output_tensors[0].len()
                    )

        import math

        # Align by 4
        top_avaialble_addrees = math.ceil(
            (float(top_avaialble_addrees) / 4) * 4)
